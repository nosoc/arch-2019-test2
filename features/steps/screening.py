...
## when part
@when("scanned by HR manager and decided to {proceed_or_not}")
def step_impl(context, proceed_or_not):
    ## updates proceed status of candidate
    context.candidate1.proceed_or_not = proceed_or_not
    ## Querying and checking        
    context.scanning_results = context.candidate1.get_candidate_outcome(context.hr1)
    query = Candidate.update(decision=context.scanning_results, proceed_or_not=proceed_or_not).where(Candidate.id == context.candidate1.id)
